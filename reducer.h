...
public:
    ...
    Reducer(
        ...
        std::size_t max_buffer_size = 500);             // How many entries before intermediate output is processed.
...
private:
    ...
    std::string output_file_name_;                      // Name of the final result file.
    std::vector<std::pair<std::string, int>> buffer_;   // Buffered final results waiting to be written.
    std::size_t max_buffer_size_;                       // Flush threshold for reducer output.
};
